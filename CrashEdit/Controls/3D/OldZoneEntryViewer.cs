        ...
        protected override void Render()
        {
            ...
            if (CheckAnchorMode())
            {
                ...
                if (anchor_sortlist)
                {
                    ...
                    if (slst != null)
                    {
                        List<OldSLSTPolygonID> polys = new(slst.Start.Polygons);
                        List<OldSLSTPolygonID> new_polys = new();
                        for (int d = 0; d + 1 <= (int)Math.Floor(anchor_cam_pos); ++d)
                        {
                            new_polys.Clear();
                            var delta = slst.Deltas[d];
                            int oldi = 0;
                            ...
                            read_rem_node();
                            read_add_node();
                            while (oldi < polys.Count || rem_left > 0 || add_left > 0)
                            {
                                if (oldi < polys.Count && rem_left > 0 && (rem_at - 1) == oldi)
                                {
                                ...
                                else if (add_left > 0 && add_at + rem_done == oldi)
                                {
                                    new_polys.Add(new((short)(add & 0x7FFF)));
                                    --add_left;
                                    ++add_done;
                                    read_add_node();
                                }
                                else if (oldi < polys.Count)
                                {
                                    int copy_amount = Math.Max(1, Math.Min(polys.Count - oldi - 1, Math.Min(add_at + rem_done - oldi, rem_at - oldi - 1)));
                                    for (int i = 0; i < copy_amount; ++i)
                                    {
                                        new_polys.Add(polys[oldi++]);
                                    }
                                }
                            }
                            ...
                            for (int i = 0; i < delta.SwapNodes.Count; ++i)
                            {
                                ...
                                swap_b += swap_a + 1;
                                var temp = new_polys[swap_a];
                                new_polys[swap_a] = new_polys[swap_b];
                                new_polys[swap_b] = temp;
                            }
