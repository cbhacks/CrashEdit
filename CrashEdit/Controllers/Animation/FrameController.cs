    ...
    public sealed class FrameController : LegacyController
    {
        ...
        public byte[] ToProcessedOBJ()
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                {
                    var model = this.AnimationEntryController.GetNSF ().GetEntry<ModelEntry>(Frame.ModelEID);
                    ...
                    foreach (ModelTransformedTriangle tri in model.Triangles)
                    {
                        ...
                        {
                            var v_n = !flip ? i : 2 - i;
                            Position vert = vertices [tri.Vertex [v_n] + Frame.SpecialVertexCount];
        ...
        public byte[] ToGameOBJ()
        {
            using (MemoryStream stream = new MemoryStream())
            {
                using (StreamWriter obj = new StreamWriter(stream))
                {
                    ...
                    foreach (Position vertex in vertices)
                    {
