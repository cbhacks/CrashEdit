    ...
    public sealed class NSFController : LegacyController
    {
        public NSFController(NSF nsf, SubcontrollerGroup parentGroup) : base(parentGroup, nsf)
        {
            ...
            else if (GameVersion == GameVersion.Crash2 || GameVersion == GameVersion.Crash3)
            {
                ...
                AddMenuSeparator ();
                AddMenu (CrashUI.Properties.Resources.NSFController_AcExportScenery, Menu_ExportSceneryOBJ);
            }
        }
        ...
        private void ExportSceneryOBJ (string path, string modelname)
        {
            ...
            foreach (SceneryEntry scenery in NSF.GetEntries<SceneryEntry> ())
            {
                ...
                foreach (var tri in scenery.Triangles)
                {
                    ...
                    var info = ProcessTextureInfoC2 (tri.Texture, tri.Animated, scenery.Textures, scenery.AnimatedTextures);
                ...
                foreach (var quad in scenery.Quads)
                {
                    ...
                    var info = ProcessTextureInfoC2 (quad.Texture, quad.Animated, scenery.Textures, scenery.AnimatedTextures);
                    string material = null;
